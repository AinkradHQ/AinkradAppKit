...
public struct AinkradStyledRun: Equatable, Sendable {
...
/// Turns process output into styled runs.
///
/// **Emits slot *indices*, never colours.** The 16 slots are mapped through the
/// host theme at render time (`AinkradANSIPalette`), so a log follows the
/// user's theme like every other surface. Hardcoding `.red` here is the thing
/// that makes a log pane look like it belongs to a different application.
///
...
/// alternate screens are what a *terminal* needs, and a log view is not one —
/// interactive sessions belong to Rune, which owns a real emulator.
///
/// Moved here from Thrall's `ThrallANSIParser`, unchanged in behaviour.
public struct AinkradANSIParser {
    ...
    public mutating func parse(_ text: String) -> [AinkradStyledRun] {
        var runs: [AinkradStyledRun] = []
        var current = ""
        ...
        while index < buffer.count {
            ...
            if buffer[cursor] == "m" {
                if !current.isEmpty {
                    runs.append(AinkradStyledRun(colorSlot: colorSlot, isBold: isBold, isDim: isDim, text: current))
                    current = ""
                }
